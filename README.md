# 👋 Hi, I'm Quang Khai

🎓 Software Developer | Frontend & Backend  
💻 Focus: Web Development & Backend Systems  
🚀 Goal: Become a strong Full-stack Developer

---

## 🧠 About Me

- 🔥 Background: Web & App Development
- 📚 Currently learning: **Backend with TypeScript (Node.js, Express, NestJS)**
- 🧩 Interested in:  
  - Backend Architecture  
  - RESTful API  
  - Database Design  
  - System Thinking
- 🎯 Career goal: **Intern / Junior Software Engineer**

---

## 🛠 Tech Stack

### Frontend
![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=for-the-badge&logo=typescript)
![TailwindCSS](https://img.shields.io/badge/TailwindCSS-38B2AC?style=for-the-badge&logo=tailwind-css)
![Vite](https://img.shields.io/badge/Vite-646CFF?style=for-the-badge&logo=vite)

### Backend
![NodeJS](https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=node.js)
![Express](https://img.shields.io/badge/Express.js-000000?style=for-the-badge&logo=express)
![CSharp](https://img.shields.io/badge/C%23-512BD4?style=for-the-badge&logo=csharp)
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python)

### Database
![SQL Server](https://img.shields.io/badge/SQL%20Server-CC2927?style=for-the-badge&logo=microsoftsqlserver)
![Firebase](https://img.shields.io/badge/Firebase-FFCA28?style=for-the-badge&logo=firebase)

### Tools
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github)
![VSCode](https://img.shields.io/badge/VSCode-007ACC?style=for-the-badge&logo=visualstudiocode)

---

## 📂 Featured Projects

### 🔹 Frontend Projects
- 🎬 **Movie Streaming App (React + TypeScript)**  
  - Fetch API, M3U8 video  
  - Favorite movies with Firebase  
  - Component-based architecture  

- 🥛 **E-commerce Website (React + Tailwind)**  
  - Responsive UI  
  - Product listing & layout design  
  - Final project for Web Programming course  

---

### 🔹 Backend Projects

- 🐍 **Backend API with Python**  
  - RESTful API  
  - Authentication & Authorization  
  - SQL Server integration  

- 🧩 **Backend System with C# (.NET / WinForms / ASP.NET MVC)**  
  - Role-based authentication  
  - Database-driven logic  
  - Real-world business workflow  

- 🚀 **Backend with Node.js (TypeScript) – In Progress**  
  - Express / NestJS  
  - Clean Architecture  
  - JWT Authentication  
  - PostgreSQL / MySQL  

---

## 📈 GitHub Stats

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=nguyenqkhai&show_icons=true&theme=tokyonight)
![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=nguyenqkhai&layout=compact&theme=tokyonight)

---

## 🧭 Learning Path

- ✔ JavaScript → TypeScript  
- ✔ React / React Native  
- ✔ SQL Server & Firebase  
- ✔ Backend Python & C#  
- 🔄 Backend TypeScript (Node.js, NestJS)  
- 🔜 System Design & DevOps basics  

---

## 📫 Contact

- 📧 Email: your-email@gmail.com  
- 💼 GitHub: https://github.com/YOUR_USERNAME  

---

⭐ *"Code with purpose. Learn by building."*
